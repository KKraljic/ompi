...
static int coll_offloaded_reduce_forced_algorithm = 0;
static int coll_offloaded_reduce_segment_size = 0;
static int coll_offloaded_reduce_max_requests;
static int coll_offloaded_reduce_tree_fanout;
static int coll_offloaded_reduce_chain_fanout;

/* valid values for coll_offloaded_reduce_forced_algorithm */
static mca_base_var_enum_value_t reduce_algorithms[] = {
    ...
    {6, "in-order_binary"},
    {7, "rabenseifner"},
    //TODO: Insert Offload within here
    {0, NULL}
};
...
int ompi_coll_offloaded_reduce_intra_check_forced_init (coll_offloaded_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    PRINT_DEBUG;
    ...
    ompi_coll_offloaded_forced_max_algorithms[REDUCE] = cnt;
    ...
    (void) mca_base_component_var_register(&mca_coll_offloaded_component.super.collm_version,
                                           ...
                                           &ompi_coll_offloaded_forced_max_algorithms[REDUCE]);

    /* MPI_T: This variable should eventually be bound to a communicator */
    coll_offloaded_reduce_forced_algorithm = 0;
    (void) mca_base_var_enum_create("coll_offloaded_reduce_algorithms", reduce_algorithms, &new_enum);
    mca_param_indices->algorithm_param_index =
        mca_base_component_var_register(&mca_coll_offloaded_component.super.collm_version,
                                        ...
                                        &coll_offloaded_reduce_forced_algorithm);
    OBJ_RELEASE(new_enum);
    if (mca_param_indices->algorithm_param_index < 0) {
        return mca_param_indices->algorithm_param_index;
    }

    coll_offloaded_reduce_segment_size = 0;
    mca_param_indices->segsize_param_index =
        mca_base_component_var_register(&mca_coll_offloaded_component.super.collm_version,
                                        ...
                                        OPAL_INFO_LVL_5,
                                        MCA_BASE_VAR_SCOPE_ALL,
                                        &coll_offloaded_reduce_segment_size);

    coll_offloaded_reduce_tree_fanout = ompi_coll_offloaded_init_tree_fanout; /* get system wide default */
    mca_param_indices->tree_fanout_param_index =
        mca_base_component_var_register(&mca_coll_offloaded_component.super.collm_version,
                                        ...
                                        OPAL_INFO_LVL_5,
                                        MCA_BASE_VAR_SCOPE_ALL,
                                        &coll_offloaded_reduce_tree_fanout);

    coll_offloaded_reduce_chain_fanout = ompi_coll_offloaded_init_chain_fanout; /* get system wide default */
    mca_param_indices->chain_fanout_param_index =
      mca_base_component_var_register(&mca_coll_offloaded_component.super.collm_version,
                                      ...
                                      OPAL_INFO_LVL_5,
                                      MCA_BASE_VAR_SCOPE_ALL,
                                      &coll_offloaded_reduce_chain_fanout);

    coll_offloaded_reduce_max_requests = 0; /* no limit for reduce by default */
...
{
    PRINT_DEBUG;
    OPAL_OUTPUT((ompi_coll_offloaded_stream,"coll:offloaded:reduce_intra_do_this selected algorithm %d topo faninout %d segsize %d",
                 algorithm, faninout, segsize));

    switch (algorithm) {
    case (0):  return ompi_coll_offloaded_reduce_intra_dec_fixed(sbuf, rbuf, count, dtype,
                                                             op, root, comm, module);
    ...
    OPAL_OUTPUT((ompi_coll_offloaded_stream,"coll:offloaded:reduce_intra_do_this attempt to select algorithm %d when only 0-%d is valid?",
                 algorithm, ompi_coll_offloaded_forced_max_algorithms[REDUCE]));
    return (MPI_ERR_ARG);
}
