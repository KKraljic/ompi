...
int ompi_coll_offloaded_dump_msg_rule (ompi_coll_msg_rule_t* msg_p)
{
    ...
    OPAL_OUTPUT((ompi_coll_offloaded_stream,"alg_id %3d\tcom_id %3d\tcom_size %3d\tmsg_id %3d\t", msg_p->alg_rule_id,
                 msg_p->com_rule_id, msg_p->mpi_comsize, msg_p->msg_rule_id));

    OPAL_OUTPUT((ompi_coll_offloaded_stream,"msg_size %10lu -> algorithm %2d\ttopo in/out %2d\tsegsize %5ld\tmax_requests %4d\n",
...
int ompi_coll_offloaded_dump_com_rule (ompi_coll_com_rule_t* com_p)
{
    ...
    if (!com_p) {
        OPAL_OUTPUT((ompi_coll_offloaded_stream,"Com rule was a NULL ptr?!\n"));
        return (-1);
    }

    OPAL_OUTPUT((ompi_coll_offloaded_stream, "alg_id %3d\tcom_id %3d\tcom_size %3d\t", com_p->alg_rule_id, com_p->com_rule_id, com_p->mpi_comsize));

    if (!com_p->n_msg_sizes) {
        OPAL_OUTPUT((ompi_coll_offloaded_stream,"no msgsizes defined\n"));
        return (0);
    }

    OPAL_OUTPUT((ompi_coll_offloaded_stream,"number of message sizes %3d\n", com_p->n_msg_sizes));

    for (i=0;i<com_p->n_msg_sizes;i++) {
        ompi_coll_offloaded_dump_msg_rule (&(com_p->msg_rules[i]));
    }
...
int ompi_coll_offloaded_dump_alg_rule (ompi_coll_alg_rule_t* alg_p)
{
    ...
    if (!alg_p) {
        OPAL_OUTPUT((ompi_coll_offloaded_stream,"Algorithm rule was a NULL ptr?!\n"));
        return (-1);
    }

    OPAL_OUTPUT((ompi_coll_offloaded_stream,"alg_id %3d\t", alg_p->alg_rule_id));

    if (!alg_p->n_com_sizes) {
        OPAL_OUTPUT((ompi_coll_offloaded_stream,"no coms defined\n"));
        return (0);
    }

    OPAL_OUTPUT((ompi_coll_offloaded_stream,"number of com sizes %3d\n", alg_p->n_com_sizes));

    for (i=0;i<alg_p->n_com_sizes;i++) {
        ompi_coll_offloaded_dump_com_rule (&(alg_p->com_rules[i]));
    }
...
int ompi_coll_offloaded_dump_all_rules (ompi_coll_alg_rule_t* alg_p, int n_rules)
{
    ...
    OPAL_OUTPUT((ompi_coll_offloaded_stream,"Number of algorithm rules %3d\n", n_rules));

    for (i=0;i<n_rules;i++) {
        ompi_coll_offloaded_dump_alg_rule (&(alg_p[i]));
    }
...
int ompi_coll_offloaded_free_msg_rules_in_com_rule (ompi_coll_com_rule_t* com_p)
{
    ...
    if (!com_p) {
        OPAL_OUTPUT((ompi_coll_offloaded_stream,"attempt to free NULL com_rule ptr\n"));
        return (-1);
    }
    ...
    if (com_p->n_msg_sizes) {
        ...
        if (!msg_p) {
            OPAL_OUTPUT((ompi_coll_offloaded_stream,"attempt to free NULL msg_rules when msg count was %d\n", com_p->n_msg_sizes));
            rc = -1; /* some error */
        }
        else {
...
int ompi_coll_offloaded_free_coms_in_alg_rule (ompi_coll_alg_rule_t* alg_p)
{
    ...
    if (!alg_p) {
        OPAL_OUTPUT((ompi_coll_offloaded_stream,"attempt to free NULL alg_rule ptr\n"));
        return (-1);
    }
    ...
    if (alg_p->n_com_sizes) {
        ...
        if (!com_p) {
            OPAL_OUTPUT((ompi_coll_offloaded_stream,"attempt to free NULL com_rules when com count was %d\n", alg_p->n_com_sizes));
        } else {
            /* ok, memory exists for the com rules so free their message rules first */
            for( i = 0; i < alg_p->n_com_sizes; i++ ) {
                com_p = &(alg_p->com_rules[i]);
                ompi_coll_offloaded_free_msg_rules_in_com_rule (com_p);
            }
...
int ompi_coll_offloaded_free_all_rules (ompi_coll_alg_rule_t* alg_p, int n_algs)
{
    ...
    for( i = 0; i < n_algs; i++ ) {
        rc += ompi_coll_offloaded_free_coms_in_alg_rule (&(alg_p[i]));
    }
 ...
 * (which can be used in the coll_offloaded_get_target_method_params() call)
 *
 */
ompi_coll_com_rule_t* ompi_coll_offloaded_get_com_rule_ptr (ompi_coll_alg_rule_t* rules, int alg_id, int mpi_comsize)
{
    ...
    OPAL_OUTPUT((ompi_coll_offloaded_stream,"Selected the following com rule id %d\n", best_com_p->com_rule_id));
    ompi_coll_offloaded_dump_com_rule (best_com_p);

    return (best_com_p);
}

/*
 * This function takes a com_rule ptr (from the communicators coll offloaded data structure)
                                              ...
                                              int* result_segsize, int* max_requests)
{
    ...
    while (i<base_com_rule->n_msg_sizes) {
        /*       OPAL_OUTPUT((ompi_coll_offloaded_stream,"checking mpi_msgsize %d against com_id %d msg_id %d index %d msg_size %d",  */
        /*             mpi_msgsize, msg_p->com_rule_id, msg_p->msg_rule_id, i, msg_p->msg_size)); */
        if (msg_p->msg_size <= mpi_msgsize) {
            best_msg_p = msg_p;
            /*          OPAL_OUTPUT((ompi_coll_offloaded_stream(":ok\n")); */
        }
        else {
            /*          OPAL_OUTPUT((ompi_coll_offloaded_stream(":nop\n")); */
            break;
        }
        /* go to the next entry */
        msg_p++;
    ...
    OPAL_OUTPUT((ompi_coll_offloaded_stream,"Selected the following msg rule id %d\n", best_msg_p->msg_rule_id));
    ompi_coll_offloaded_dump_msg_rule (best_msg_p);
